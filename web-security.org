...
attacker to *read arbitrary files from the web server*.

Path traversal is typically presented as an attack against confidentiality, but
it can also be used to break integrity: *By uploading a file with name
../index.php*, the attacker might be able to take full control of the web
application logic!
...
- use an existing API to *canonicalize the filename*, i.e., to uniquely resolve it
  into an absolute path
- perform the security checks over the canonicalized filename, e.g., *check that
    it starts with the expected base directory*

** Command injection
Command injection is an infamous web security vulnerability which allows the
attacker to execute *arbitrary OS commands on the server*.

Since command injection leverages a call to an OS command, most of the times the
attacker is forced to *play blind*. In particular, the following side-channels are
...
3) *DNS resolution*, e.g., nslookup canary.attacker.com
...
- If this is not possible, *sanitize your input by using existing libraries*,
  e.g., shlex.quote in Python escapes shell metacharacters;
- *Separate commands from parameters by using the APIs* of your programming
  language, e.g., subprocess.run in Python

** Templates
Server-side programming languages often use templates: Templates might suffer
from *XSS vulnerabilities*, though web frameworks often provide auto-escaping
features.
...
#+begin_src php
$output = $twig->render($_GET[’custom_message’],
...
#+begin_quote
*Serialization*: The process of converting complex data structures into a “flat”
...
- damage can be done even before deserialization is finished, e.g., exceptions
  leading to DoS or abuse of magic methods like wakeup
- weakly typed programming languages are particularly at risk, yet strong typing
...
- the loose comparison operator == can equate two elements with different types
- we have 0 == "Any string not starting with numbers"
- this only works when the serialized object contains 0 as integer, not
as string

*** Impact and defenses
Insecure deserialization may have dangerous consequences:
1) remote code execution;
2) privilege escalation and authentication bypasses
3) denial of service
4) data-only attacks

To prevent insecure deserialization:
- *do not serialize user-controlled data*!
- if you really need to do that, sign the serialized data and *check their
    integrity* before attempting deserialization
- do not serialize objects: use pure data format like *JSON*
...
- HTML: enabler for persistent XSS
- SVG: supports inline JavaScript

*** Preventing UFU
Unrestricted file upload is complicated to deal with, hence also difficult to
detect automatically:
1) sanitize the content of the filename (for application integrity)
2) file extensions cannot be trusted, but white-listing them is useful because
   some programs require specific extensions to operate
3) check / sanitize / re-encode the content of the uploaded files
4) restrict upload operations to authenticated users
5) put the uploaded files out of the webroot / on an external domain, so that
   SOP can provide protection
6) put a limit on the file size to avoid denial of service.
...
- typical targets: the local host or other back-end servers sitting on the same
  local network, protected by a firewall

Why do we need server-side requests in web applications?
*Data import*, *Previews*, *Chaching*.

SSRF can also be abused to:
1) *Attack remote servers*: the confused back-end is fooled into sending malicious
   requests to other remote servers, so that the attack is not coming from the
   attacker’s machine;
2) *Bypass SOP*: the confused back-end is fooled into fetching malicious content
   from the attacker’s server, hence the attacker gets scripting capabilities in
   the server’s origin

*** Preventing SSRF: whitelisting
...
- or at least disable XXE support: many modern XML parsing libraries already do
...
- if the API gives you back a list of parameters, you already have all the
...
* OAuth 2.0
We would like to have a way for Internet users to grant websites or applications
access to their information on other websites, but without giving them access to
their passwords!

#+begin_quote
*OAuth*: The OAuth 2.0 authorization framework enables a third-party application
 to obtain limited access to an HTTP service on behalf of a resource owner, by
 orchestrating an approval interaction between the resource owner and the HTTP
 service.
#+end_quote

** Entities
- *Resource Owner*: the user who authorizes a client application to access their
  account;
- *Client Application*: the web app that wants to access the user’s account, which
  is hosted at the resource server;
- *Resource Server*: the web app that hosts the user’s account and the associated
  resources;
- *Authorization Server*: the web app that is in charge of verifying the user’s
  identity and granting authorization.

** OAuth flow
1) Registration form on the service’s website, providing at least: *Client name*,
   *Homepage URL*, *Redirect URL* / *Callback URL*
2) Once the client is registered, the service will issue client credentials to it: *client identifier*, *client secret*
3) the authorization grant type depends on the method used by the client to request authorization and on the grant types supported by the service
4) After authenticating at the service, the user is prompted to authorize or
   deny access to their account by the client
5) If the user provides consent, the service redirects the browser to the
   redirect URL which was specified during the client registration, along with
   an *authorization code*
6) The client requests an *access token* from the service, by passing the
   *authorization code* along with *authentication credentials*, including the
   client secret, to the API token endpoint
7) If the authorization code is valid, the service will send a response with the
   access token to the client. Now the client is authorized

   The step of the Authorization Code grant may include an optional *refresh token*,
   which allows the client to obtain a new authentication token without
   involving the user again

*** Implicit grant type
The *implicit grant flow* is intended for use cases where the confidentiality of
the client secret is hard to achieve. It basically works as follows: the user is
asked to authorize the client, then the service passes the access token back to
the user-agent, which passes it to the application.

If the user provides consent, the service redirects the browser to the redirect
URL along with the access token (no authorization code here).

The *Implicit flow has no authorization code and the access token is sent* to
the browser before passing it to the client. The Implicit flow does not
authenticate the identity of the client and relies on the redirect URL to serve
this purpose.

#+begin_quote
*Once the access token is leaked, e.g., via XSS, the attacker can operate under
 the user’s identity until the token expires*!
#+end_quote

** OAuth Security
*** CSRF
if CitySearch does not implement CSRF protection on the review form, a web
attacker can abuse SSO to post reviews in Alice’s name both on CitySearch and
Facebook, thus amplifying the attack;

*** Session swapping
Even when all the sensitive forms are protected against CSRF, a broken
implementation of OAuth might suffer from a session swapping attack. Its effects
are equivalent to login CSRF.

*** Forced profile linking
Clients supporting authentication using both local passwords and SSO via OAuth
may implement a profile linking functionality to connect two accounts of the
same user, so that either set of credentials can be used:
#+begin_src
https://client.com/oauth-linking?code=CODE
#+end_src

1) The attacker runs OAuth in their browser and gets a *valid authorization code*
2) The attacker forges the *profile linking request* from the victim’s browser,
   sending their own code
3) The attacker authenticates using OAuth and gets access to the *victim’s account*

*** Countermeasure: State parameter
The optional state parameter of OAuth can be used to mitigate CSRF.
1) At the start of the run, the client generates a random state parameter
2) The value of state is saved in the user’s session and shared with the service
3) If the value of state at step 7 does not match the one in the session, the
   run is aborted -> *The attacker’s state does not work in the victim’s session*

*** Token stealing attacks
Token stealing attacks may take different forms. A paradigmatic example is
enabled by a lenient validation of the redirect URL implemented by the service,
e.g., white-listing all the subdomains of a given client.
1) The attacker starts the protocol from the victim’s browser, setting the
   redirect URL at step 3 to https://evil.wordpress.com
2) The attacker gets access to the victim’s code when it is sent to the redirect
   URL at step 7
3) The attacker sends the stolen code to blog.wordpress.com and authenticates as
   Alice from their browser

*** Mitigation: Strict Validation
Token stealing attacks can be mitigated by performing strict validation of the
redirect URL, i.e., perform exact matching of pre-registered URLs and it aborts
the protocol in case of mismatch.

*** Migitation: PKCE
Proof Key for Code Exchange (PKCE) is an OAuth extension designed to mitigate
the risks of stolen authorization codes.
- Extension of the Authorization Code Grant that requires the client to generate
  a random secret known as the *code verifier*, which is saved in the session;
- A hash of the code verifier known as the *code challenge* is sent in the request
  for the auth code and bound to the code at the service;
- The client must provide the code verifier when exchanging the auth code for
  the access token

** OAuth security best practices
- *Implement PKCE* as an effective countermeasure against some of the most
  significant attacks against OAuth;
- *Perform strict validation of the redirect URL*, using different URLs for
  different services with explicit user intention tracking;
- *Implement defenses against CSRF*, e.g., using one-time tokens bound to the user
  agent in the *state parameter*
- Do not use OAuth for SSO alone, but consider OpenID Connect
- Limit the scope and the duration of access tokens
